...
<%
    ...
    if (userEmail == null){ userEmail = "doe@rutgers.edu";} //filler
    ...
    try {
        ...
        if ("makeReservation".equals(action)){
            ...
            if (rs.next()) {
                ...
                emailFromDB = "doe@rutgers.edu"; //filler
            }
            rs.close();
            stmt.close();
            String countSql = "SELECT COUNT(*) AS total FROM reservationHas";
            stmt = conn.prepareStatement(countSql);
            rs = stmt.executeQuery();
            int rnumber = 1;
            if (rs.next()) rnumber = rs.getInt("total") + 1;
            rs.close();
            stmt.close();
            ...
            String insertSql = "INSERT INTO reservationHas (rnumber, date, totalFare, transitLine, passenger, email) VALUES (?, ?, ?, ?, ?, ?)";
            stmt = conn.prepareStatement(insertSql);
            stmt.setInt(1, rnumber);
            stmt.setString(2, dateStr + " 00:00:00");
            stmt.setDouble(3, fare);
            stmt.setString(4, lineNameFound);
